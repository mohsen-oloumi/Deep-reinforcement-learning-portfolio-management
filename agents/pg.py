...
class PG:
    def __init__(self, M, L, N, name, load_weights, trainable, type, number):
        ...
        if trainable == 'True':
            # Initial summary
            self.summary_writer = tf.summary.FileWriter('./result/PG/' + self.number + '/' + 'summary/' + type + '/',
                                                        self.sesson.graph)
    ...
    def train(self):
        ...
        profit, _ = self.sesson.run([self.profit, self.optimize],
                                    feed_dict={self.state: s, self.out: np.reshape(a, (-1, self.M)),
