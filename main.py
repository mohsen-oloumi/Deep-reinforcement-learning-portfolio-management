...
class StockTrader():
    ...
    def write(self, codes, agent):
        ...
        history.to_csv(
            PATH_prefix + agent + '-'.join(codes) + '-' + str(math.exp(np.sum(self.r_history)) * 100) + '.csv')
...
def backtest(agent, env):
    ...
    print('asset name', '   ', 'Average daily rate of return', '   ', 'sharp', '   ', 'Maximum drawdown')
...
def session(config, args):
    ...
    if framework == 'DDPG':
        ...
        agent = DDPG(predictor, (codes) + 1, int(window_length), len(features), '-'.join(agent_config), reload_flag,
                     trainable)
    ...
    if args['mode'] == 'train':
        ...
        with open("result/PG/" + str(args['num']) + '/config.json', 'r') as f:
            dict_data = json.load(f)
        test_start_date, test_end_date, codes = datetime.datetime.strptime(dict_data['test_start_date'],
                                                                           '%Y-%m-%d'), datetime.datetime.strptime(
        ...
        backtest([PG(len(codes) + 1, int(window_length), len(features), '-'.join(agent_config), 'True', 'False', 'True',
                     args['num'])],
                 env)
